...
def reset_supabase_db():
    ...
    # ------------------------------
    # Insert Users_Ext with one user linked to 3 companies
    # ------------------------------
    ...
    multi_company_user = user_id_map["meitner"]

    for username, user_id in user_id_map.items():
        if username == "einstein":
            users_ext_data.append({"user_id": user_id, "role_id": role_id_map["Super Admin"], "company_id": None, "is_active": True, "notes": None})
        elif any(user_id == c["owner_id"] for c in inserted_companies):
            company_id = next(c["id"] for c in inserted_companies if c["owner_id"] == user_id)
            users_ext_data.append({"user_id": user_id, "role_id": role_id_map["Owner"], "company_id": company_id, "is_active": True, "notes": None})
        elif not assigned_admin:
            users_ext_data.append({"user_id": user_id, "role_id": role_id_map["Admin"], "company_id": company_ids[0], "is_active": True, "notes": None})
            assigned_admin = True
        elif user_id == multi_company_user:
            for cid in company_ids[:3]:
                users_ext_data.append({"user_id": user_id, "role_id": role_id_map["Client"], "company_id": cid, "is_active": True, "notes": None})
        else:
            for company_id in company_ids:
                if company_id not in assigned_clients:
                    users_ext_data.append({"user_id": user_id, "role_id": role_id_map["Client"], "company_id": company_id, "is_active": True, "notes": None})
                    assigned_clients.add(company_id)
                    break
    ...
    for i in range(15):
        ...
        services_data.append({
            "users_id": user_ids[i % len(user_ids)],
            "task_id": 1,
    ...
    services_data.extend([
        {"users_id": marie_id, "task_id": 1, "ml_method_id": 1, "device_id": 1, "company_id": company_map["Greyhound Dynamics"], "start_date": "2024-01-01", "end_date": "2025-12-31"},
        {"users_id": marie_id, "task_id": 1, "ml_method_id": 1, "device_id": 2, "company_id": company_map["Poodle Analytics"], "start_date": "2024-01-01", "end_date": "2025-12-31"}
    ])
    ...
    # ------------------------------
    results_data = []
    mode_cycle = [3, 3, 1, 2]  # Increase frequency of mode 3
